...
DEFAULTS = {
    "output_dir": "artifacts/runs/telemetry/raw/ablation/hypercube/m25_emergent_bridi",
    "run_id": "",
...
def _run_m25_emergent_bridi_suite(**context: object) -> None:
    ...
    run_repo_script(
        "scripts/m25/run_m25_emergent_bridi_suite.py",
        [
            ...
            "--generator-epochs", str(int(cfg.get("generator_epochs", 64))),
            "--advisor-epochs", str(int(cfg.get("advisor_epochs", 64))),
            "--prompt-epochs", str(int(cfg.get("prompt_epochs", 64))),
            "--batch-size", str(int(cfg.get("batch_size", 512))),
            "--max-symbols", str(int(cfg.get("max_symbols", 32))),
            "--symbol-budget", str(int(cfg.get("symbol_budget", 16))),
...
with DAG(
    dag_id="lojban_m25_emergent_bridi",
    description="M25 emergent loose bridi grammar-action stream suite.",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["lojban", "m-series", "m25", "emergent-bridi"],
    params={
    ...
    PythonOperator(
        task_id="run_m25_emergent_bridi_suite",
        python_callable=_run_m25_emergent_bridi_suite,
    )
