...
DEFAULTS = {
    ...
    "upstream_bridge_suite": "",
    "upstream_m11_manifest": "",
    "pack_jsonl": "",
    "strict_balance": True,
    "output_dir": "artifacts/runs/telemetry/raw/ablation/hypercube/m3_18_decoder_reentry_resume",
    "local_files_only": False,
    "run_id": "",
}


def _run_m3_18(**context: object) -> None:
    ...
    args = [
        ...
        "--d-train-objective", str(cfg.get("d_train_objective", "continuation_ce")),
        "--continuation-target-max-tokens", str(int(cfg.get("continuation_target_max_tokens", 5))),
        "--residual-guard-threshold", str(float(cfg.get("residual_guard_threshold", 0.01))),
    ...
    for key, flag in (("upstream_bridge_suite", "--upstream-bridge-suite"), ("upstream_m11_manifest", "--upstream-m11-manifest")):
    ...
    run_repo_script("scripts/run_m3_18_decoder_reentry_resume.py", args)


with DAG(
    dag_id="lojban_m3_18_decoder_reentry_resume",
    description="M3.18 decoder re-entry resume: train and evaluate compressed decoder-native return channels after advisor reasoning.",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["lojban", "m-series", "m3-18", "reentry", "resume", "ablation"],
    params={
        ...
        "d_train_objective": Param("continuation_ce", type="string", enum=["margin", "continuation_ce"]),
        "continuation_target_max_tokens": Param(5, type="integer", minimum=1),
        "residual_guard_threshold": Param(0.01, type="number", minimum=0.0),
        ...
        "upstream_bridge_suite": Param("", type="string"),
        "upstream_m11_manifest": Param("", type="string"),
        "pack_jsonl": Param("", type="string"),
        "strict_balance": Param(True, type="boolean"),
        "output_dir": Param("artifacts/runs/telemetry/raw/ablation/hypercube/m3_18_decoder_reentry_resume", type="string", minLength=1),
        "local_files_only": Param(False, type="boolean"),
        "run_id": Param("", type="string"),
    },
) as dag:
    PythonOperator(
        task_id="run_m3_18_decoder_reentry_resume",
        python_callable=_run_m3_18,
    )
