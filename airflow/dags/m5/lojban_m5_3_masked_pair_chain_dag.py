...
DEFAULTS = {
    ...
    "layer_index": 12,
    "max_chain_steps": 4,
    "brivi_gate_alpha": 4.0,
    "brivi_gate_tau": 0.25,
    "l_output_dir": "runs/l_series/m5_3_masked_pair_chain",
    "report_output_dir": "artifacts/runs/telemetry/raw/ablation/hypercube/m5_3_masked_pair_chain",
    "local_files_only": False,
}


def _run_m5_3(**context: object) -> None:
    ...
    args = [
        ...
        "--brivi-gate-alpha",
        str(float(cfg.get("brivi_gate_alpha", 4.0))),
        "--brivi-gate-tau",
        str(float(cfg.get("brivi_gate_tau", 0.25))),
        "--l-output-root",
    ...
    run_repo_script("scripts/run_m5_3_masked_pair_chain.py", args)


with DAG(
    dag_id="lojban_m5_3_masked_pair_chain",
    description="M5.3 masked minimal-pair autoregressive matrix-chain isolated test.",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["lojban", "m-series", "m5-3", "masked-pair-chain"],
    params={
        ...
        "brivi_gate_alpha": Param(4.0, type="number"),
        "brivi_gate_tau": Param(0.25, type="number"),
        "l_output_dir": Param("runs/l_series/m5_3_masked_pair_chain", type="string", minLength=1),
        "report_output_dir": Param("artifacts/runs/telemetry/raw/ablation/hypercube/m5_3_masked_pair_chain", type="string", minLength=1),
        "local_files_only": Param(False, type="boolean"),
    },
) as dag:
    PythonOperator(
        task_id="run_m5_3_masked_pair_chain",
        python_callable=_run_m5_3,
    )
