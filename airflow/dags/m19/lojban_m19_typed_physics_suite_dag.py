...
def _run_suite(**context: object) -> None:
    ...
    args = [
        ...
        "--typed-slot-layout",
        str(cfg.get("typed_slot_layout", "gismu:2,cmavo:2,judri:4")),
        "--arity-router-mode",
        str(cfg.get("arity_router_mode", "")),
        "--geometry-mode",
        str(cfg.get("geometry_mode", "")),
        "--poincare-curvature",
        str(float(cfg.get("poincare_curvature", 1.0))),
        "--run-id",
        run_id,
    ]
    if bool(cfg.get("gumbel_hard", False)):
        args.append("--gumbel-hard")
    run_repo_script("scripts/m19/run_m19_typed_physics_suite.py", args)


with DAG(
    dag_id="lojban_m19_typed_physics_suite",
    description="Train and evaluate typed-physics M19 branches.",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["lojban", "m-series", "m19", "typed-physics"],
    params={
        "track": Param("M19.31", type="string", minLength=1),
        ...
        "typed_slot_layout": Param("gismu:2,cmavo:2,judri:4", type="string", minLength=1),
        "arity_router_mode": Param("", type="string"),
        "gumbel_hard": Param(False, type="boolean"),
        "geometry_mode": Param("", type="string"),
        "poincare_curvature": Param(1.0, type="number", minimum=0.00001),
        "run_id": Param("", type="string"),
    },
) as dag:
    PythonOperator(
        task_id="run_m19_typed_physics_suite",
