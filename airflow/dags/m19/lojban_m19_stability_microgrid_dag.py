from __future__ import annotations

from datetime import datetime

from airflow import DAG
from airflow.models.param import Param
from airflow.operators.python import PythonOperator

from lojban_airflow_utils import merge_conf, run_repo_script, sanitize_run_id, validate_output_partition


DEFAULTS = {
    "base_model": "C:/Users/Andrew/hf_models/Qwen2.5-0.5B-Instruct",
    "data_path": "artifacts/datasets/m19_mixed_curriculum_v1.jsonl",
    "eval_data_path": "artifacts/datasets/m14_5_unified/m14_5_unified_test.jsonl",
    "audit_data_path": "artifacts/datasets/sanity_check_v1.jsonl",
    "eval_size": 400,
    "audit_eval_size": 10,
    "epochs": 1,
    "learning_rate_list": "5e-5,1e-4",
    "augmentation_prob_list": "0.0,0.5",
    "format_augmentation_prob_list": "0.0",
    "seed_list": "23,29",
    "surface_consistency_weight_list": "",
    "surface_consistency_task_weight": 0.15,
    "pointer_necessity_weight_list": "",
    "pointer_necessity_weight": 0.0,
    "pointer_necessity_margin": 0.05,
    "pointer_necessity_ablation_mode": "no_judri",
    "checkpoint_selection_policy": "final_only",
    "selection_surface_size": 200,
    "selection_regimes": "",
    "track": "M19",
    "cell_id": "M19.3_8Q_128D_8S",
    "num_queries": 8,
    "bottleneck_dim": 128,
    "scratchpad_length": 8,
    "max_latent_steps": 64,
    "random_scale": 0.05,
    "output_dir": "artifacts/runs/telemetry/raw/ablation/hypercube/m19_stability_microgrid",
    "run_id": "",
}


def _run_m19_stability_microgrid(**context: object) -> None:
    dag_run = context.get("dag_run")
    conf = getattr(dag_run, "conf", None)
    cfg = merge_conf(DEFAULTS, conf)
    output_dir = validate_output_partition(str(cfg.get("output_dir", "")), "telemetry/raw")
    run_id = sanitize_run_id(str(cfg.get("run_id") or getattr(dag_run, "run_id", "manual")))
    args = [
        "--base-model",
        str(cfg.get("base_model")),
        "--data-path",
        str(cfg.get("data_path")),
        "--eval-data-path",
        str(cfg.get("eval_data_path")),
        "--audit-data-path",
        str(cfg.get("audit_data_path")),
        "--eval-size",
        str(int(cfg.get("eval_size", 400))),
        "--audit-eval-size",
        str(int(cfg.get("audit_eval_size", 10))),
        "--epochs",
        str(int(cfg.get("epochs", 1))),
        "--learning-rate-list",
        str(cfg.get("learning_rate_list", "5e-5,1e-4")),
        "--augmentation-prob-list",
        str(cfg.get("augmentation_prob_list", "0.0,0.5")),
        "--format-augmentation-prob-list",
        str(cfg.get("format_augmentation_prob_list", "0.0")),
        "--seed-list",
        str(cfg.get("seed_list", "23,29")),
        "--surface-consistency-weight-list",
        str(cfg.get("surface_consistency_weight_list", "")),
        "--surface-consistency-task-weight",
        str(float(cfg.get("surface_consistency_task_weight", 0.15))),
        "--pointer-necessity-weight",
        str(float(cfg.get("pointer_necessity_weight", 0.0))),
        "--pointer-necessity-weight-list",
        str(cfg.get("pointer_necessity_weight_list", "")),
        "--pointer-necessity-margin",
        str(float(cfg.get("pointer_necessity_margin", 0.05))),
        "--pointer-necessity-ablation-mode",
        str(cfg.get("pointer_necessity_ablation_mode", "no_judri")),
        "--checkpoint-selection-policy",
        str(cfg.get("checkpoint_selection_policy", "final_only")),
        "--selection-surface-size",
        str(int(cfg.get("selection_surface_size", 200))),
        "--selection-regimes",
        str(cfg.get("selection_regimes", "")),
        "--track",
        str(cfg.get("track", "M19")),
        "--cell-id",
        str(cfg.get("cell_id", "M19.3_8Q_128D_8S")),
        "--num-queries",
        str(int(cfg.get("num_queries", 8))),
        "--bottleneck-dim",
        str(int(cfg.get("bottleneck_dim", 128))),
        "--scratchpad-length",
        str(int(cfg.get("scratchpad_length", 8))),
        "--max-latent-steps",
        str(int(cfg.get("max_latent_steps", 64))),
        "--random-scale",
        str(float(cfg.get("random_scale", 0.05))),
        "--output-root",
        output_dir,
        "--run-id",
        run_id,
    ]
    run_repo_script("scripts/m19/run_m19_stability_microgrid.py", args)


with DAG(
    dag_id="lojban_m19_stability_microgrid",
    description="Weak-seed stabilization micro-grid for the active M19 mainline.",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["lojban", "m-series", "m19", "stability"],
    params={
        "base_model": Param("C:/Users/Andrew/hf_models/Qwen2.5-0.5B-Instruct", type="string", minLength=1),
        "data_path": Param("artifacts/datasets/m19_mixed_curriculum_v1.jsonl", type="string", minLength=1),
        "eval_data_path": Param("artifacts/datasets/m14_5_unified/m14_5_unified_test.jsonl", type="string", minLength=1),
        "audit_data_path": Param("artifacts/datasets/sanity_check_v1.jsonl", type="string", minLength=1),
        "eval_size": Param(400, type="integer", minimum=1),
        "audit_eval_size": Param(10, type="integer", minimum=1),
        "epochs": Param(1, type="integer", minimum=1),
        "learning_rate_list": Param("5e-5,1e-4", type="string", minLength=1),
        "augmentation_prob_list": Param("0.0,0.5", type="string", minLength=1),
        "format_augmentation_prob_list": Param("0.0", type="string", minLength=1),
        "seed_list": Param("23,29", type="string", minLength=1),
        "surface_consistency_weight_list": Param("", type="string"),
        "surface_consistency_task_weight": Param(0.15, type="number", minimum=0.0),
        "pointer_necessity_weight": Param(0.0, type="number", minimum=0.0),
        "pointer_necessity_weight_list": Param("", type="string"),
        "pointer_necessity_margin": Param(0.05, type="number", minimum=0.0),
        "pointer_necessity_ablation_mode": Param("no_judri", type="string", minLength=1),
        "checkpoint_selection_policy": Param("final_only", type="string", minLength=1),
        "selection_surface_size": Param(200, type="integer", minimum=1),
        "selection_regimes": Param("", type="string"),
        "track": Param("M19", type="string", minLength=1),
        "cell_id": Param("M19.3_8Q_128D_8S", type="string", minLength=1),
        "num_queries": Param(8, type="integer", minimum=1),
        "bottleneck_dim": Param(128, type="integer", minimum=1),
        "scratchpad_length": Param(8, type="integer", minimum=1),
        "max_latent_steps": Param(64, type="integer", minimum=1),
        "random_scale": Param(0.05, type="number", minimum=0.0),
        "output_dir": Param("artifacts/runs/telemetry/raw/ablation/hypercube/m19_stability_microgrid", type="string", minLength=1),
        "run_id": Param("", type="string"),
    },
) as dag:
    PythonOperator(task_id="run_m19_stability_microgrid", python_callable=_run_m19_stability_microgrid)
