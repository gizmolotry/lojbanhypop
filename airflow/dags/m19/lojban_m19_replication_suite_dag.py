...
DEFAULTS = {
    ...
    "seed_list": "19,23,29",
    "entity_rename_augmentation_prob": 0.0,
    "format_flatten_augmentation_prob": 0.0,
    "surface_consistency_weight": 0.0,
    "surface_consistency_task_weight": 0.0,
    ...
    "max_latent_steps": 64,
    "random_scale": 0.05,
    "output_dir": "artifacts/runs/telemetry/raw/ablation/hypercube/m19_replication_suite",
    "run_id": "",
}


def _run_m19_replication(**context: object) -> None:
    ...
    args = [
        ...
        "--learning-rate",
        str(float(cfg.get("learning_rate", 0.0001))),
        "--seed-list",
        str(cfg.get("seed_list", "19,23,29")),
        "--entity-rename-augmentation-prob",
        str(float(cfg.get("entity_rename_augmentation_prob", 0.0))),
        "--format-flatten-augmentation-prob",
        str(float(cfg.get("format_flatten_augmentation_prob", 0.0))),
        "--surface-consistency-weight",
        str(float(cfg.get("surface_consistency_weight", 0.0))),
        "--surface-consistency-task-weight",
        str(float(cfg.get("surface_consistency_task_weight", 0.0))),
    ...
    run_repo_script("scripts/m19/run_m19_replication_suite.py", args)


with DAG(
    dag_id="lojban_m19_replication_suite",
    description="Seed replication suite for the active M19 mainline.",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["lojban", "m-series", "m19", "replication"],
    params={
        ...
        "learning_rate": Param(0.0001, type="number", minimum=0.0),
        "seed_list": Param("19,23,29", type="string", minLength=1),
        "entity_rename_augmentation_prob": Param(0.0, type="number", minimum=0.0),
        "format_flatten_augmentation_prob": Param(0.0, type="number", minimum=0.0),
        "surface_consistency_weight": Param(0.0, type="number", minimum=0.0),
        "surface_consistency_task_weight": Param(0.0, type="number", minimum=0.0),
        ...
        "output_dir": Param("artifacts/runs/telemetry/raw/ablation/hypercube/m19_replication_suite", type="string", minLength=1),
        "run_id": Param("", type="string"),
    },
) as dag:
    PythonOperator(task_id="run_m19_replication_suite", python_callable=_run_m19_replication)
