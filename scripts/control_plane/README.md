# Control Plane Scripts

These scripts maintain the canonical research control plane.

## Responsibilities

- backfill historical ablation records
- build family maps and ordered lineage spines
- render human-readable ledgers and catalogs
- support orchestration-facing reporting flows

If a script exists to explain, normalize, aggregate, or render the research program as a whole, it belongs here.
