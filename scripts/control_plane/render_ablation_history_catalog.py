...
def _render_entry(entry: dict[str, Any]) -> list[str]:
    ...
    comparison_contract = entry.get("comparison_contract", {})
    if isinstance(comparison_contract, dict):
        targets = [
            str(target.get("target", ""))
            for target in comparison_contract.get("comparison_targets", [])
            if isinstance(target, dict) and str(target.get("target", "")).strip()
        ]
        if targets:
            lines.append(f"- Automatic compare-against: `{', '.join(targets)}`")
        required_tests = comparison_contract.get("required_test_contract_ids", [])
        if required_tests:
