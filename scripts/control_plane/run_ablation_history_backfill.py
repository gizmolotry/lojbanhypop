...
from lojban_evolution.control_plane.path_registry import (
    canonical_repo_path as shared_canonical_repo_path,
    canonicalize_path_list as shared_canonicalize_path_list,
)
...
M18_SURFACE_MINOR: dict[str, str] = {
    "sapir_whorf_audit": "0",
    "harmonized_audit": "1",
    "hybrid_cot_audit": "2",
}

M18_SURFACE_CELL_CODES: dict[str, dict[str, str]] = {
    "harmonized_audit": {
        "EN-CONCISE": "A",
        "EN-COT": "B",
        "ZH-COT": "C",
...
LETTER_SERIES_SPECS: list[dict[str, Any]] = [
    {
        ...
        "script_paths": ["scripts/legacy/run_coconut_ablation_matrix.py"],
        "dag_paths": ["airflow/dags/control_plane/lojban_ablation_matrix_dag.py"],
    },
    {
        "series_key": "H",
    ...
    },
    {
        ...
        "script_paths": [
            "scripts/legacy/train_h5_persistent_vq_advisor.py",
            "scripts/legacy/eval_h5_dynamic_pointer_refactor.py",
            "scripts/legacy/eval_h5_ood_stress.py",
            "archive/legacy_scaffolding/trace_h5_provenance.py",
        ],
        "dag_paths": [],
    },
    {
        ...
        "script_paths": [
            "archive/legacy_scaffolding/eval_j_1.py",
            "archive/legacy_scaffolding/eval_j_2.py",
            "archive/legacy_scaffolding/eval_j_3.py",
            "archive/legacy_scaffolding/eval_j_4.py",
            "archive/legacy_scaffolding/eval_j_5.py",
            "scripts/legacy/train_h5_persistent_vq_advisor.py",
        ],
        "dag_paths": ["airflow/dags/legacy/lojban_j_series_dag.py"],
    },
    {
        "series_key": "L",
        "title": "L series",
        ...
        "dag_paths": ["airflow/dags/legacy/lojban_l_series_dag.py"],
    },
    {
        ...
        "script_paths": [],
        "dag_paths": ["airflow/dags/control_plane/lojban_ablation_hypercube_report_dag.py"],
    },
    {
        ...
        "dag_paths": ["airflow/dags/control_plane/lojban_phase_ablation_dag.py"],
    },
]
...
def main() -> None:
    ...
    entries = _canonicalize_entry_paths(entries)
    series_family_manifests = _build_letter_series_manifests(entries)
    series_family_manifests = _canonicalize_series_family_manifests(series_family_manifests)
    ...
    manifest = {
        ...
        "series": series_metadata("M", "M.ablation_history_backfill", "scripts/control_plane/run_ablation_history_backfill.py"),
        ...
        "comparison_contracts": build_comparison_index(taxonomy),
...
def _collect_doc_claims(
    ...
    baseline_entry = ensure_entry(
        ...
        script_paths=["scripts/run_phase5_two_stage_recovery.py", "scripts/data/mine_compositional_anchors.py"],
        notes=[
...
def _collect_telemetry_reports(registry: dict[str, dict[str, Any]], source_paths: set[str]) -> None:
    ...
    for path in root.rglob("*.json"):
        ...
        if not track:
            continue
        report_script_paths: list[str] = []
        report_dag_paths: list[str] = []
        series = payload.get("series")
        if isinstance(series, dict) and str(series.get("script") or "").strip():
            report_script_paths.append(str(series.get("script")).strip())
        family_contract = payload.get("family_contract")
        if isinstance(family_contract, dict):
            runner_script = str(family_contract.get("runner_script") or "").strip()
            dag_path = str(family_contract.get("dag") or "").strip()
            if runner_script:
        ...
        if track == "M11.discriminative":
            _register_family_level_manifest(
                ...
                path=path,
                timestamp=str(payload.get("timestamp") or ""),
                script_paths=["scripts/m11/run_m11_discriminative_suite.py"],
        ...
        if isinstance(payload.get("cells"), dict):
            for cell_name, cell_payload in payload.get("cells", {}).items():
                ...
                if track == "M18" and surface in M18_SURFACE_MINOR:
                    ...
                    canonical_id = f"m.track.m18_{M18_SURFACE_MINOR[surface]}.{str(cell_code).lower()}"
                else:
                ...
                entry = ensure_entry(
                    ...
                    title=title,
                    aliases=[f"{track}.{surface}.{cell_name}" if surface else f"{track}.{cell_name}", str(cell_name)],
                    lookup_aliases=[f"{track}.{surface}.{cell_name}" if surface else f"{track}.{cell_name}"],
                    objective=f"telemetry-rooted {track} ablation cell",
                    baseline_relation=f"{track} cell",
                    script_paths=report_script_paths,
                    dag_paths=report_dag_paths,
                    artifact_roots=[_path_str(path.parent)],
                )
                add_evidence(
                    entry,
        ...
        if metrics:
            _register_family_level_manifest(
                ...
                script_paths=sorted(set(p for p in report_script_paths if p)),
                dag_paths=sorted(set(p for p in report_dag_paths if p)),
            )
...
def _command_scripts(command: Any) -> list[str]:
    ...
    for part in command:
        ...
        text = part.replace("\\", "/")
        if text.endswith(".py"):
            output.append(_canonical_repo_path(text))
    return sorted(set(output))


def _canonical_repo_path(value: str) -> str:
    return shared_canonical_repo_path(value, Path(__file__).resolve().parents[2])


def _canonicalize_path_list(paths: list[str]) -> list[str]:
    return shared_canonicalize_path_list(paths, Path(__file__).resolve().parents[2])


def _canonicalize_entry_paths(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    for entry in entries:
        entry["script_paths"] = _canonicalize_path_list(list(entry.get("script_paths", [])))
        entry["dag_paths"] = _canonicalize_path_list(list(entry.get("dag_paths", [])))
    return entries


def _canonicalize_series_family_manifests(manifests: list[dict[str, Any]]) -> list[dict[str, Any]]:
    for manifest in manifests:
        manifest["script_paths"] = _canonicalize_path_list(list(manifest.get("script_paths", [])))
        manifest["dag_paths"] = _canonicalize_path_list(list(manifest.get("dag_paths", [])))
    return manifests
