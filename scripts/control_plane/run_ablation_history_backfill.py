...
def _canonical_repo_path(value: str) -> str:
    ...
    repo_root = Path(__file__).resolve().parents[2].as_posix().rstrip("/")
    relative = normalized
    if normalized.startswith(repo_root + "/"):
        relative = normalized[len(repo_root) + 1 :]
    return PATH_CANONICALIZATION.get(relative, relative)


def _canonicalize_path_list(paths: list[str]) -> list[str]:
    return sorted({_canonical_repo_path(path) for path in paths if str(path).strip()})
