...
def main():
    ...
    try:
        while True:
            ...
            op_vector, x_probs, op_idx = s1.build_graph(premise_state, prompt_len)
            pos_hypothesis = op_vector # [1, H]
            
            # 2. Generate Negative Hypotheses (Hallucination/Noise)
            # Use the M9HardNegativeGenerator for adversarial contrast
            neg_hypotheses = neg_gen.generate(premise_state, op_vector, x_probs, op_idx) # [1, 5, H]
            ...
            if step % 500 == 0:
                # Periodic Checkpoint
                output_dir = Path("archive/results/m9/active/RESULTS_M9_FORGE")
