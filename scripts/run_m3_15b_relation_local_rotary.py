...
def _build_winograd_pack(size: int, seed: int, strict_balance: bool = True) -> list[dict[str, Any]]:
    return _materialize_pack(_pair_variants(_WINOGRAD_PAIR_BANK), size=size, seed=seed, strict_balance=strict_balance, item_prefix="m3_15b")
...
def _build_winograd_split_packs(size: int, seed: int, strict_balance: bool = True) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, list[str]]]:
    ...
    train_pack = _materialize_pack(_pair_variants(train_pairs), size=size, seed=seed, strict_balance=strict_balance, item_prefix="m3_15b_train")
    val_pack = _materialize_pack(_pair_variants(val_pairs), size=size, seed=seed + 101, strict_balance=strict_balance, item_prefix="m3_15b_val")
    eval_pack = _materialize_pack(_pair_variants(eval_pairs), size=size, seed=seed + 202, strict_balance=strict_balance, item_prefix="m3_15b_eval")
...
def _evaluate_cell(
    ...
    for item in eval_pack:
        ...
        with torch.no_grad():
            ...
            rel_nodes = base_states[:, 0::3, :]
            eval_align, eval_sim = bridge.alignment_metrics(local_hidden, rel_nodes)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="M3.15b Relation-Local Rotary Bridge: candidate-local alignment to advisor predicate geometry.")
    ...
    p.add_argument("--output-root", type=Path, default=Path("artifacts/runs/telemetry/raw/ablation/hypercube/m3_15b_relation_local_rotary"))
...
def main() -> None:
    ...
    (run_dir / "m3_15b_A_eval.json").write_text(json.dumps(rows_a, indent=2), encoding="utf-8")
    print(f"A: {met_a}")
    ...
    (run_dir / "m3_15b_B_eval.json").write_text(json.dumps(rows_b, indent=2), encoding="utf-8")
    print(f"B: {met_b}")
    ...
    (run_dir / "m3_15b_C_eval.json").write_text(json.dumps(rows_c, indent=2), encoding="utf-8")
    ...
    (run_dir / "m3_15b_B_seed2_eval.json").write_text(json.dumps(rows_b_seed2, indent=2), encoding="utf-8")
    ...
    (run_dir / "m3_15b_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    md = [
    ...
    (run_dir / "m3_15b_report.md").write_text("\n".join(md), encoding="utf-8")
    print(f"M3.15b complete: {run_dir}")
