...
def run_suite(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for seed in seeds:
        result = train_m24_substrate_compression(
            ...
            active_frame_budget=int(args.active_frame_budget) if int(args.active_frame_budget) > 0 else None,
            trace_symbol_budget=int(args.trace_symbol_budget) if int(args.trace_symbol_budget) > 0 else None,
    ...
    payload = {
        ...
        "config": {
            ...
            "mdl_weight": float(args.mdl_weight),
            "trace_exact_surrogate_weight": float(args.trace_exact_surrogate_weight),
            "active_frame_budget": int(args.active_frame_budget),
            "trace_symbol_budget": int(args.trace_symbol_budget),
        },
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--active-frame-budget", type=int, default=0, help="Maximum active bridi frames retained; 0 disables the hard frame bottleneck.")
    parser.add_argument("--trace-symbol-budget", type=int, default=0, help="Maximum emitted packed trace symbols retained; 0 disables the hard symbol bottleneck.")
