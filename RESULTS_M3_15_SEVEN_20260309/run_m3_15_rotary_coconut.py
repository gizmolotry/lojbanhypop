...
def _build_winograd_pack(size: int, seed: int, strict_balance: bool = True) -> list[dict[str, Any]]:
    ...
    # Minimal pairs: variant 0 and 1 have opposite gold_index.
    ...
    while len(out) < tgt:
        ...
        if strict_balance:
            ...
            if g == 1 and c1 > c0:
                continue
        row["item_id"] = f"m3_15_{len(out):06d}"
        out.append(row)
        if g == 0:
            c0 += 1
        else:
...
def main() -> None:
    ...
    pack_train = _load_pack(args.pack_jsonl, size=int(args.eval_size), seed=int(args.seed), strict_balance=bool(args.strict_balance))
    pack_eval = _load_pack(args.pack_jsonl, size=int(args.eval_size), seed=int(args.seed) + 1, strict_balance=bool(args.strict_balance))
    ...
    report["cells"]["A"] = {"train": train_a, "metrics": met_a}
    ...
    report["cells"]["B"] = {"train": train_b, "metrics": met_b}
    (run_dir / "m3_15_B_eval.json").write_text(json.dumps(rows_b, indent=2), encoding="utf-8")
    print(f"B: {met_b}")

    runtime_enabled = float(met_b["overall_accuracy"]) >= float(met_a["overall_accuracy"]) + float(args.runtime_enable_min_acc_gain)
    # C
