...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run isolated M5.3 masked-pair matrix-chain test.")
    ...
    p.add_argument("--l-output-root", type=Path, default=Path("runs/l_series/m5_3_masked_pair_chain"))
    p.add_argument("--report-output-root", type=Path, default=Path("artifacts/runs/telemetry/raw/ablation/hypercube/m5_3_masked_pair_chain"))
...
def main() -> None:
    ...
    l_run_root = args.l_output_root / ts / "m5_3_smoke"
    report_root = args.report_output_root / f"m5_3_{ts}"
    ...
    trainer = Path(__file__).resolve().parent / "train_m5_3_masked_pair_chain.py"
    cmd = [
        sys.executable,
        str(trainer),
        "--base-model",
        ...
        "--brivi-gate-alpha",
        str(float(args.brivi_gate_alpha)),
        "--brivi-gate-tau",
        str(float(args.brivi_gate_tau)),
    ]
    ...
    summary_path = latest / "m5_3_masked_pair_chain_summary.json" if latest is not None else None
    payload: dict[str, Any] = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "series": series_metadata("M", "M5.3.masked_pair_chain.run", "scripts/run_m5_3_masked_pair_chain.py"),
    ...
    report_json = report_root / "m5_3_masked_pair_chain_report.json"
    report_md = report_root / "m5_3_masked_pair_chain_report.md"
    report_json.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    lines = [
        "# M5.3 Masked Pair Chain",
        "",
    ...
    if final_step:
        lines.extend(
            [
                ...
                f"- mean_step_grounding_score: {final_step.get('mean_step_grounding_score', 'n/a')}",
                f"- mean_brivi_gate: {final_step.get('mean_brivi_gate', 'n/a')}",
                f"- ungrounded_op_rate: {final_step.get('ungrounded_op_rate', 'n/a')}",
