...
class StructuralAlignmentBridge(torch.nn.Module):
    ...
    def runtime_cue(self, eng_hidden: torch.Tensor, max_nodes: int, cue_norm_cap: float) -> torch.Tensor:
        e_nodes = self.project_english_nodes(eng_hidden, max_nodes=max_nodes)
        ...
        if cap > 0.0:
            ...
            scale = torch.clamp(cap / n, max=1.0)
            cue = cue * scale
        return cue
...
def _evaluate_cell(
    ...
    for item in eval_ds:
        with torch.no_grad():
            ...
            if cell == "C":
                cue = bridge.runtime_cue(
                    eng_hidden,
                    max_nodes=int(args.max_nodes),
                    cue_norm_cap=float(args.runtime_cue_norm_cap),
                )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--runtime-gate-cap", type=float, default=0.0, help="If >0, clamp runtime cue gate for cell C.")
    p.add_argument("--runtime-cue-norm-cap", type=float, default=0.0, help="If >0, clip cue norm for cell C.")
...
def main() -> None:
    ...
    report: dict[str, Any] = {
        ...
        "series": series_metadata("M", "M3.14", "scripts/run_m3_14_structural_alignment_bridge.py"),
        "track": "M3.14",
