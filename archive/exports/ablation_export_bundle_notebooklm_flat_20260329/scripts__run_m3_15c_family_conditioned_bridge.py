...
class FamilyConditionedRotaryBridge(torch.nn.Module):
    def __init__(self, hidden_size: int, relation_vocab: int, max_nodes: int = 12) -> None:
        ...
        self.family_embed = torch.nn.Embedding(len(FAMILY_TO_ID), hidden_size)
        self.family_head = torch.nn.Linear(hidden_size, len(FAMILY_TO_ID), bias=False)
    ...
    def _family_vec(self, family_ids: torch.Tensor, *, dim: int | None = None) -> torch.Tensor:
        fam = self.family_embed(family_ids)
        if fam.dim() == 2:
            fam = fam.unsqueeze(1)
        if dim is not None and int(fam.shape[-1]) != int(dim):
            if int(fam.shape[-1]) > int(dim):
                fam = fam[..., : int(dim)]
            else:
                fam = F.pad(fam, (0, int(dim) - int(fam.shape[-1])))
        return fam

    def _phase_alignment(self, local_nodes: torch.Tensor, adv_nodes: torch.Tensor, family_ids: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if family_ids is not None:
            e = e + self._family_vec(family_ids, dim=int(e.shape[-1]))
    ...
    def relation_bias(self, local_nodes: torch.Tensor, adv_rel_nodes: torch.Tensor, family_ids: torch.Tensor | None = None) -> torch.Tensor:
        en = local_nodes
        an = adv_rel_nodes
    ...
    def runtime_cue(self, local_nodes: torch.Tensor, cue_norm_cap: float, family_ids: torch.Tensor | None = None) -> torch.Tensor:
        pooled = torch.mean(self.eng_proj(local_nodes), dim=1, keepdim=True)
        if family_ids is not None:
            pooled = pooled + self._family_vec(family_ids, dim=int(pooled.shape[-1]))
    ...
    def alignment_metrics(self, local_nodes: torch.Tensor, adv_rel_nodes: torch.Tensor, family_ids: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        return self._phase_alignment(local_nodes, adv_rel_nodes, family_ids=family_ids)

    def family_logits(self, local_nodes: torch.Tensor) -> torch.Tensor:
        pooled = torch.mean(self.eng_proj(local_nodes), dim=1)
        return self.family_head(pooled)
...
def _build_winograd_pack(size: int, seed: int, strict_balance: bool = True) -> list[dict[str, Any]]:
    return _materialize_pack(_pair_variants(_WINOGRAD_PAIR_BANK), size=size, seed=seed, strict_balance=strict_balance, item_prefix="m3_15c")
...
def _build_winograd_split_packs(size: int, seed: int, strict_balance: bool = True) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, list[str]]]:
    ...
    train_pack = _materialize_pack(_pair_variants(train_pairs), size=size, seed=seed, strict_balance=strict_balance, item_prefix="m3_15c_train")
    val_pack = _materialize_pack(_pair_variants(val_pairs), size=size, seed=seed + 101, strict_balance=strict_balance, item_prefix="m3_15c_val")
    eval_pack = _materialize_pack(_pair_variants(eval_pairs), size=size, seed=seed + 202, strict_balance=strict_balance, item_prefix="m3_15c_eval")
...
def _family_id_tensor(model, item: dict[str, Any]) -> torch.Tensor:
    family = str(item.get("family", "adjective_property"))
    family_id = int(FAMILY_TO_ID.get(family, 0))
    return torch.tensor([family_id], device=_model_device(model), dtype=torch.long)


def _train_bridge_cell(
    cell: str,
    bridge: FamilyConditionedRotaryBridge,
    model,
    tokenizer,
    ...
    for step in range(int(args.train_steps)):
        ...
        family_ids = _family_id_tensor(model, item)

        with torch.no_grad():
            _eng_hidden, local_hidden, _local_meta = _extract_relation_local_states(
                model,
                tokenizer,
                prompt,
                candidates,
        ...
        opt.zero_grad()
        align_loss, align_sim = bridge.alignment_metrics(local_hidden, rel_nodes, family_ids=family_ids)
        cue = bridge.runtime_cue(local_hidden, cue_norm_cap=float(args.runtime_cue_norm_cap), family_ids=family_ids)
        ...
        rb = None
        if cell == "C":
            raw_rb = bridge.relation_bias(local_hidden, rel_nodes, family_ids=family_ids)
            rb = _scaled_relation_bias(raw_rb, float(args.relation_bias_scale))
        family_loss = F.cross_entropy(bridge.family_logits(local_hidden), family_ids)
        ...
        loss = (
            ...
            + float(args.family_loss_weight) * family_loss
            + reg
        )
        loss.backward()
        opt.step()
        ...
        family_hist.append(float(family_loss.detach().item()))
    ...
    return {
        ...
        "family_loss": float(sum(family_hist) / max(1, len(family_hist))),
...
def _evaluate_cell(
    ...
    for item in eval_pack:
        ...
        family_ids = _family_id_tensor(model, item)
        with torch.no_grad():
            eng_hidden, local_hidden, local_meta = _extract_relation_local_states(
                model,
                tokenizer,
                prompt,
                candidates,
            ...
            rel_nodes = base_states[:, 0::3, :]
            eval_align, eval_sim = bridge.alignment_metrics(local_hidden, rel_nodes, family_ids=family_ids)
            align_eval_hist.append(float(eval_align.item()))
            sim_eval_hist.append(float(eval_sim.item()))
            fam_pred = torch.argmax(bridge.family_logits(local_hidden), dim=-1)
            family_acc_hist.append(float((fam_pred == family_ids).float().mean().item()))
    ...
    metrics = {
        ...
        "family_classification_accuracy": float(sum(family_acc_hist) / max(1, len(family_acc_hist))),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="M3.15c Family-Conditioned Bridge: family-aware local bridge usefulness ablation.")
    ...
    p.add_argument("--family-loss-weight", type=float, default=0.5)
    ...
    p.add_argument("--output-root", type=Path, default=Path("artifacts/runs/telemetry/raw/ablation/hypercube/m3_15c_family_conditioned_bridge"))
...
def main() -> None:
    ...
    (run_dir / "m3_15c_eval_pack_preview.json").write_text(json.dumps(pack_eval[:20], indent=2), encoding="utf-8")
    ...
    report: dict[str, Any] = {
        ...
        "series": series_metadata("M", "M3.15c", "scripts/run_m3_15c_family_conditioned_bridge.py"),
        "track": "M3.15c",
    ...
    # A
    bridge_a = FamilyConditionedRotaryBridge(hidden, int(args.relation_vocab), max_nodes=int(args.max_nodes)).to(target_device, dtype=module_dtype)
    ...
    (run_dir / "m3_15c_A_eval.json").write_text(json.dumps(rows_a, indent=2), encoding="utf-8")
    print(f"A: {met_a}")

    # B
    bridge_b = FamilyConditionedRotaryBridge(hidden, int(args.relation_vocab), max_nodes=int(args.max_nodes)).to(target_device, dtype=module_dtype)
    ...
    (run_dir / "m3_15c_B_eval.json").write_text(json.dumps(rows_b, indent=2), encoding="utf-8")
    print(f"B: {met_b}")

    runtime_enabled = float(met_b_val["overall_accuracy"]) >= float(met_a_val["overall_accuracy"]) + float(args.runtime_enable_min_acc_gain)
    # C
    bridge_c = FamilyConditionedRotaryBridge(hidden, int(args.relation_vocab), max_nodes=int(args.max_nodes)).to(target_device, dtype=module_dtype)
    ...
    (run_dir / "m3_15c_C_eval.json").write_text(json.dumps(rows_c, indent=2), encoding="utf-8")
    ...
    (run_dir / "m3_15c_B_seed2_eval.json").write_text(json.dumps(rows_b_seed2, indent=2), encoding="utf-8")
    ...
    (run_dir / "m3_15c_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    md = [
    ...
    md.extend(
        [
            ...
            f"- family_loss_weight: `{float(args.family_loss_weight):.3f}`",
            "",
    ...
    (run_dir / "m3_15c_report.md").write_text("\n".join(md), encoding="utf-8")
    print(f"M3.15c complete: {run_dir}")
